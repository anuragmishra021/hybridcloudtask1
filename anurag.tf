...
resource "aws_instance" "web1" {
 ...
 connection {
    type     = "ssh"
    user     = "ec2-user"
    private_key = file("C:/Users/Anurag/Downloads/anuargkey1.pem")
...
resource "null_resource" "image" {
  provisioner "local-exec" {
    command = "git clone https://github.com/anuragmishra021/hybridcloudtask1.git images"
  }
}
...
//make s3 bucket
resource "aws_s3_bucket" "anurag_1211" {
  bucket = "awsanurag1211"
  acl = "public-read"
...
resource "aws_s3_bucket_object" "object" {
  bucket = aws_s3_bucket.anurag_1211.bucket
  acl = "public-read"
  key    = "terraform.png"
  source = "images/terraform.png"
depends_on=[aws_s3_bucket.anurag_1211,null_resource.image]

}
//make cloudfront distribution
resource "aws_cloudfront_distribution" "my_distribution" {
    origin {
         domain_name = "${aws_s3_bucket.anurag_1211.bucket_regional_domain_name}"
         origin_id   = "${aws_s3_bucket.anurag_1211.id}"
...
default_cache_behavior {
    ...
    target_origin_id = "${aws_s3_bucket.anurag_1211.id}"
...
restrictions {
 ...
 depends_on=[aws_s3_bucket.anurag_1211]
}
...
provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/anuragmishra021/hybridcloudtask1.git /var/www/html/",
      "sudo su << EOF",
